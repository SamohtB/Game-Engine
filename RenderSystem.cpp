...
RenderSystem::RenderSystem() 
{
	...
	for (UINT driver_type_index = 0; driver_type_index < num_driver_types;)
	{
		res = D3D11CreateDevice(NULL, driver_types[driver_type_index], NULL, NULL, feature_levels, num_feature_levels,
			D3D11_SDK_VERSION, &m_d3d_device, &m_feature_levels, &m_imm_context);
	...
	if (FAILED(res))
	{
		throw std::exception("CreateDevice not created successfully");
	}
...
RenderSystem::~RenderSystem()
{
	...
	if (m_imm_device_context) delete m_imm_device_context;
}
...
bool RenderSystem::compileVertexShader(const wchar_t* fileName, const char* entry_point_name, void** shader_byte_code, size_t* byte_code_size)
{
	ID3DBlob* errBlob = nullptr;
	if (!SUCCEEDED(::D3DCompileFromFile(fileName, nullptr, nullptr, entry_point_name, "vs_5_0", 0, 0, &m_blob, &errBlob)))
	{
		if (errBlob)
		{
			errBlob->Release();
...
bool RenderSystem::compilePixelShader(const wchar_t* fileName, const char* entry_point_name, void** shader_byte_code, size_t* byte_code_size)
{
	ID3DBlob* errBlob = nullptr;
	if (!SUCCEEDED(::D3DCompileFromFile(fileName, nullptr, nullptr, entry_point_name, "ps_5_0", 0, 0, &m_blob, &errBlob)))
	{
		if (errBlob)
		{
			errBlob->Release();
